...
async function main() {
  ...
  const chatCompletion = await groq.chat.completions.create({
    "messages": [
      ...
      {role: "user",name:"[🌌_system", content:"Source d'information sur G7 : url (https://www.consilium.europa.eu/fr/meetings/international-summit/2024/06/13-15/) "},
      ...
      {role: "assistant",name:"adoPi", content: "rédiger un rapport complet du G7 en Italie Concernant le climat et le facteur économique EMP(1378 de la commission cnccfp.fr) ce rappart sera transmis au juge des élections européennes." },
      {role: "user",content: " Rédige le rapport au format HTML index.html le rapport section par section attestant nos sources respectant les normes du Web sémantique W3C" },
    ...
    const outputFilePath = "G7_EMP-📚_" + new Date().toISOString().replace(/[-:TZ]/g, "") + ".html";
    fs.writeFileSync(outputFilePath, htmlContent);
    console.log("📚 Rapport généré et enregistré dans " + outputFilePath);
});
}
